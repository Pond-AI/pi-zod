...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@pond-ai/pi-coding-agent")).toBe("Run: pnpm install -g @pond-ai/pi-coding-agent");
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@pond-ai/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@pond-ai/pi-coding-agent")).toBe(
			"Update @pond-ai/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@pond-ai/pi-coding-agent");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "@pond-ai/pi-coding-agent"],
			display: `npm --prefix ${prefix} install -g @pond-ai/pi-coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@pond-ai/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "@pond-ai/pi-coding-agent"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g @pond-ai/pi-coding-agent`);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@pond-ai/pi-coding-agent")).toBe("Run: npm install -g @pond-ai/pi-coding-agent");
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "@pond-ai/pi-coding-agent"],
			display: "bun install -g @pond-ai/pi-coding-agent",
		});
	});
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@pond-ai/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@pond-ai/pi-coding-agent")).toContain(
